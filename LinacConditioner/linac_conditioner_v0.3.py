...
sys.path.append('C:\\Put a directory on PYTHONPATH here\\Software\\striptool')
...
class linac_condition(QObject):
    ...
    def __init__(self, parent = None):
        ...
        self.increase_wait_time = self.wait_time = 240; # Time between RF step increments (in seconds)
        self.permit_RF_drop    = 1000; # RF step drop if RF permit lost
        self.rf_upper_limit    = 100000 # AW suggestion
        self.maximum_pulse_width = 2; # Absolute mas pulse length
    ...
    def check_RF_permit_is_good(self, rf_permit_signal, rf_trig_signal, rf_amp_signal):
        ...
        if (self.latest_values[rf_permit_signal] != self.pv_monitor_not_alarm_values[rf_permit_signal]):
            ...
            self.rf_trig_lost = False
        if self.rf_dropped:
            self.dont_panic_text = "RF DROPPED!"
    ...
    def get_breakdown_rate(self):    
        ...
        return self.breakdown_per_minute
            
    def writelog(self, type):
        if type == 'vacuum':
            with open(self.log_file,"a") as logfile:
                logfile.write( str(time.time()) + '\t' + 'Vacuum spike' + '\t' + str(self.breakdown_count) + '\t' + str(self.get_breakdown_rate()) + '\t' + str(self.normal_RF_increase) + '\n')           
        elif type == 'ref':
            with open(self.log_file,"a") as logfile:
                logfile.write( str(time.time()) + '\t' + 'Reflected power spike' + '\t' + str(self.breakdown_count) + '\t' + str(self.get_breakdown_rate()) + '\t' + str(self.normal_RF_increase) + '\n')          
    ...
    def check_IMG_change_is_small(self, img_signal, rf_amp_signal, rf_pulse_width_signal):
        ...
        if self.delta_IMG > self.pv_monitor_not_alarm_values[img_signal]:
            ...
            self.reset_rf_value = self.latest_values['RF_AMPLITUDE'] - 100
            ...
            while self.delta_IMG > self.pv_monitor_not_alarm_values[img_signal]:
                ...
                time.sleep(2)
                self.latest_values = getCurrentValues(pv_to_monitor)
                self.delta_IMG = self.latest_values[img_signal] - self.pre_spike_img_mean
            self.change_RF_amp(rf_amp_signal, reset_rf_value)
            writelog('vacuum')
            self.dont_panic_text = "Normal Running"
    ...
    def check_LLRF_rev_power_is_low(self, ref_signal, rf_amp_signal, rf_pulse_width_signal):
        ...
        if self.latest_values[ref_signal] is not None:
            ...
            if (max_rev_power - self.rev_pwr_mean) > self.pv_monitor_not_alarm_values[ref_signal]:
                if self.is_not_in_cool_down(ref_signal):
                    ...
                    self.breakdown_count += 1
                    self.reset_rf_value = self.latest_values[rf_amp_signal]- 100
                    self.change_RF_amp(rf_amp_signal, 0)
    ...
    def main_loop(self):
        ...
        if (time.time() - self.loop_start_time) % 60 < 0.01:
            print 'Breakdown rate is ' + str(self.get_breakdown_rate()) + ' breakdowns/minute (' + str(self.get_breakdown_rate() * 30) + ' breakdowns/30mins with ' + str(self.breakdown_count) + ' breakdowns)'
...
class linac_condition_view_widget(QWidget):
    ...
    def update(self):
        val = getattr(self.gc, self.param)
