...
class Reader:
    ...
    def __init__(self, name):
        ...
        self.Machine_Mode = self.search("MACHINE_MODE")
        ...
        self.Gun_Type = self.search("Gun_Type")
        self.BPM_Type = self.search("BPM_Type")
        ...
        self.LLRF = "LLRF"
        self.Laser = "LASER"
        self.VariableTypeList = ["MAGNETS", "LLRF", "LASER", "DAQ"]
        ...
        # self.DAQ_Laser_On = self.search("DAQ_LASER_ON")
        # self.DAQ_Laser_Off = self.search("DAQ_LASER_OFF")
        self.DAQKeywords = ["DAQ_LASER_ON", "DAQ_LASER_OFF"]
        self.Using_Laser = self.search("USING_LASER")
        self.Using_Llrf = self.search("USING_LLRF")
        self.Using_BPM = self.search("USING_BPM")
        self.Using_BPMKywords = ["VELA_INJ", "CLARA_INJ",]
        self.Using_Mag = self.search("USING_MAG")
        self.Machine_Mode = self.search("MACHINE_MODE")
        self.True_False_Offline = ["T","F", "O"]

    def isPhysical(self):
        if self.Machine_Mode == "VIRTUAL": # use virtual machine
            return False
        elif self.Machine_Type == "PHYSICAL": # use Physical machine
            return True
    ...
    def LoopData(self, k):
        with open(self.name + ".txt") as input_data:
            dict = {}
            ...
            for line in input_data:  # This keeps reading the file
                ...
                if not newwords:  # gets rid of the empty lines
                    ...
                    pass  # ignores comments in txt file
                else:
