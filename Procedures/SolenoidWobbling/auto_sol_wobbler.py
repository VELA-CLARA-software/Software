...
sol2_pv = epics.PV('CLA-GUN-MAG-SOL-02:SETI')

nominal_current = 0 # the nominal setting of the solenoid Max 250, (or maybe 300)
Amplitude  = 250 # +- Amps from nominal_current (peak of sin wave)
...
if sol2_pv.get() < (nominal_current):
	while sol2_pv.get() < (nominal_current):
		sol2_pv.put( sol2_pv.get() + 1 )
		time.sleep( 0.5*wait_time )
elif sol2_pv.get() > (nominal_current):
	while sol2_pv.get() > (nominal_current):
		sol2_pv.put( sol2_pv.get() - 1 )
		time.sleep( 0.5*wait_time )
