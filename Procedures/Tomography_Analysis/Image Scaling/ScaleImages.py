...
for image, i in zip(images, range(len(images))):
    ...
    # scale each image
    imresized = cv2.resize(image, None, fx=magnfcnx / math.sqrt((betaxobs[i])),
                           fy=magnfcny / (math.sqrt(betayobs[i])), interpolation=cv2.INTER_CUBIC)
    ...
    imscaled[(x2 - smallestx):(x2 + smallestx), (y2 - smallesty):(y2 + smallesty)] = imresized[(
                                                                                                           x1 - smallestx):(
                                                                                                           x1 + smallestx),
                                                                                     (
                                                                                                 y1 - smallesty):(
...
# export imagearray to hd5 file
data_file = h5py.File('imagearray.hdf5', 'w')
data_file.create_dataset('imagearray', data=imagearray)
data_file.close()
