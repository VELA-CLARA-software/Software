...
def plots(data1, popt1, data2, popt2, final_image):
...
def get_beam_centered_image(input_image, col_center, row_center, return_width, return_height):
    '''
    takes imaged_data and crops it around row_centre, and col_centre to give an image of
    return_width and return_height pixels
    TODO: add exceptions if we are cropping outside the input_image
    :param input_image: data to crop
    :param col_center: central column of cropped image
    :param row_center:  central row of cropped image
    :param return_width: cropped image width
    :param return_height: cropped image height
    :return: cropped image
    '''

    half_return_width = return_width * 0.5
    half_return_height= return_height * 0.5

    start_col = int(round(col_center - half_return_width))
    start_row = int(round(row_center - half_return_height))
    end_col = int(round(col_center + half_return_width))
    end_row = int(round(row_center + half_return_height))

    # check we're not cropping outside the original image

    if start_col < 0:
        start_col = 0
        print("Warning! start_col < 0")
    if start_row < 0:
        print("Warning! start_row < 0")
        start_row = 0

    if end_col >= len(input_image[0]):
        print("Warning! end_col >= " + str(len(input_image[0])))
        end_col = len(input_image[0]) - 1
    if end_row < 0:
        print("Warning! end_row >= " + str(len(input_image)))
        end_row = len(input_image) - 1

    return input_image[start_row:end_row, start_col:end_col]
...
# now we crop image_to_fit based on teh beam center
final_image = get_beam_centered_image(input_image=image_to_fit, col_center=fit_cols[2],
                                      row_center=fit_rows[2], return_width=1200, return_height=1200)



plots(sum_cols, fit_cols, sum_rows, fit_rows, final_image)
