...
""" LOUISE: Ky fwd Upper limit, LLRF interlock CLA-GUN-LRF-CTRL-01:app:interlock:state, save fwd power trace """
...
class linac_condition(QObject):
    ...
    def __init__(self, parent = None):
        ...
        self.last_10_fwd_pwr_values = []
        self.last_10_amp_values = []
        self.last_10_trig_values = []
        # where to write the log
        self.log_file = "gunbreakdowns.txt"
        self.rev_trace_log = "gunbreakdown_trace.txt"
        self.probe_trace_log = "gunbreakdown_trace_probe.txt"
        # toggle actualy doing conditioning!
        self.doConditioning = False
        self.incooldown = False

        # tweakable parameters
        self.normal_RF_increase = 100;  # RF step increment
        self.increase_wait_time = self.wait_time = 1*60; # Time between RF step increments (in seconds)
        self.permit_RF_drop    = 1000; # RF step drop if RF permit lost
        self.rf_upper_limit    = 15200  # Based on 8/9/17 data, should be about 10MW
        ...
        self.pv_to_monitor = {
            ...
            'LLRF_CAVITY_REV_POWER' : epics.PV('CLA-GUN-LRF-CTRL-01:ad1:ch4:power_remote.POWER'),
            'RF_PULSE_LENGTH'       : epics.PV('CLA-GUN-LRF-CTRL-01:vm:feed_fwd:duration'),
            'IMG_1'                 : epics.PV('CLA-LRG1-VAC-IMG-01:P'), #needs changing to correct IMG
            'RF_AMPLITUDE'          : epics.PV('CLA-GUN-LRF-CTRL-01:vm:dsp:sp_amp:amplitude'),
            'LLRF_KLY_FWD_PWR'      : epics.PV('CLA-GUN-LRF-CTRL-01:ad1:ch1:power_remote.POWER'),
            'LLRF_CAVITY_FWD_PWR'   : epics.PV('CLA-GUN-LRF-CTRL-01:ad1:ch3:power_remote.POWER'),
            'LLRF_PROBE_FWD_PWR'    : epics.PV('CLA-GUN-LRF-CTRL-01:ad1:ch6:power_remote.POWER'),
        ...
        self.pv_monitor_not_alarm_values= {
            ...
            'IMG_1': 3E-9,  # This is the level above background at which the pulse width drops
            'LLRF_CAVITY_FWD_PWR': 0.8,
            'LLRF_CAVITY_FWD_PWR_LOW':0.1
        }
    ...
    def getInitialValues(self):
        ...
        self.rev_pwr_mean = max( epics.PV('CLA-GUN-LRF-CTRL-01:ad1:ch4:power_remote.POWER').get() )
        self.fwd_pwr_mean = sum( epics.PV('CLA-GUN-LRF-CTRL-01:ad1:ch4:power_remote.POWER').get() )
        self.amp_mean = epics.PV('CLA-GUN-LRF-CTRL-01:ad1:ch4:power_remote.POWER').get()
        self.trig_mean = epics.PV('CLA-GUN-HRF-MOD-01:FAULT').get()
        for i in range(11):
            ...
            self.last_10_fwd_pwr_values.append(self.fwd_pwr_mean)
            self.last_10_amp_values.append(self.amp_mean)
        for i in range(6):
            self.last_10_trig_values.append(self.trig_mean)
    ...
    def writetrace(self, filename='', ref_signal='LLRF_CAVITY_REV_POWER'):
        tracestring = ""
    ...
    def check_IMG_change_is_small(self, img_signal, rf_amp_signal, rf_pulse_width_signal):
        ...
        # for i in self.last_10_trig_values:
        #     if i != 3:
        #         self.last_10_img_values = []
        #         self.pre_spike_img_mean = self.latest_values[img_signal]
        ...
        if not self.incooldown and self.delta_IMG > self.pv_monitor_not_alarm_values[img_signal]:
            print 'delta IMG = ', self.delta_IMG
            ...
            self.vac_count += 1
            self.monitor_Signal.emit(['vac_count', self.vac_count])
            ...
            self.writelog('vacuum')
            self.monitor_Signal.emit(['dont_panic_text', "Normal Running"])
            time.sleep(0.3)  # delays for 0.2 seconds
        elif self.incooldown == False and self.rf_dropped == False:
    ...
    def check_LLRF_rev_power_is_low(self, ref_signal, rf_amp_signal, rf_pulse_width_signal):
        ...
        #print "rev power start of loop" + str(self.last_10_trig_values)
        #for i in self.last_10_trig_values:
        #    if i != 3:
         #       print "in new trig loop (Rev)" + str(self.last_10_trig_values)
         #       self.last_10_rev_pwr_values = []
          #      self.rev_pwr_mean = max(self.latest_values[ref_signal])
          #      print "rev_pwr_mean = " + str(self.rev_pwr_mean)
        if len(self.last_10_rev_pwr_values) > 10:
           self.last_10_rev_pwr_values.pop(0)
        if self.latest_values[ref_signal] is not None:
            ...
            if not self.incooldown and max_rev_power  > self.pv_monitor_not_alarm_values[ref_signal]*self.rev_pwr_mean:
                if self.is_not_in_cool_down(ref_signal):
                    ...
                    self.change_RF_amp(rf_amp_signal, self.reset_rf_value)
                    self.writelog('ref')
                    self.writetrace(filename=self.rev_trace_log, ref_signal='LLRF_CAVITY_REV_POWER')
                    self.writetrace(filename=self.probe_trace_log, ref_signal='LLRF_PROBE_FWD_PWR')
                    self.dont_panic_text = "Normal Running"
                    time.sleep(0.3) # delays for 0.3 second
            elif self.incooldown == False and self.rf_dropped == False:
                    #print "ref power loop in cooldown = " + str(self.incooldown) + " rf dropped = " + str(self.rf_dropped)
    ...
    def check_LLRF_fwd_power_for_drop(self, fwd_signal,fwd_signal_low,amp_signal):
        # for i in self.last_10_trig_values:
        #     if i != 3:
        #         self.last_10_fwd_pwr_values = []
        #         self.last_10_amp_values = []
        #         self.fwd_pwr_mean =sum(self.latest_values[fwd_signal])
        #         self.amp_mean =self.latest_values[amp_signal]
        if len(self.last_10_fwd_pwr_values) > 10:
           self.last_10_fwd_pwr_values.pop(0)
        if len(self.last_10_amp_values) > 10:
           self.last_10_amp_values.pop(0)
        if self.latest_values[fwd_signal] is not None:
            #print 'latest values ref power', max( latest_values['LLRF_CAVITY_REV_POWER'])
            sum_fwd_power = sum( self.latest_values[fwd_signal] )
            if not self.incooldown and not self.rf_dropped and self.pv_monitor_not_alarm_values[fwd_signal_low]*self.fwd_pwr_mean < sum_fwd_power < self.pv_monitor_not_alarm_values[fwd_signal]*self.fwd_pwr_mean:
                if self.latest_values[amp_signal] >= self.amp_mean:
                    print str(self.latest_values[amp_signal]) + "must be more than or equal" + str(self.amp_mean)
                    if self.is_not_in_cool_down(fwd_signal):
                        print 'Waveguide breakdown?'
                        # time.sleep(0.3) # delays for 0.3 second
            elif self.incooldown == False and self.rf_dropped == False:
                #print "fwd power loop in cooldown = " + str(self.incooldown) + " rf dropped = " + str(self.rf_dropped)
                self.last_10_fwd_pwr_values.append(sum_fwd_power)
                self.fwd_pwr_mean = numpy.mean(self.last_10_fwd_pwr_values)
                self.monitor_Signal.emit(['fwd_pwr_mean', self.fwd_pwr_mean])
                self.last_10_amp_values.append(self.latest_values[amp_signal])
                self.amp_mean = numpy.mean(self.last_10_amp_values)

    def can_increase_rf(self):
        if self.latest_values['RF_AMPLITUDE'] < self.rf_upper_limit and self.doConditioning:
    ...
    def update_RF_trig_buffer(self):
        self.last_10_trig_values.append(epics.PV('CLA-GUNS-HRF-MOD-01:Sys:StateSet').get())
        if len(self.last_10_trig_values) > 5:
            self.last_10_trig_values.pop(0)
        #print "trig buffer updater" +str(self.last_10_trig_values)
    ...
    def main_loop(self):
        ...
        # print 'rep rate = ', 1.0/(time.time() - self.start_time)
        self.start_time = time.time()
        ...
        self.check_LLRF_fwd_power_for_drop('LLRF_CAVITY_FWD_PWR','LLRF_CAVITY_FWD_PWR_LOW', 'RF_AMPLITUDE')
        ...
        if self.can_increase_rf():
            ...
            print 'Increasing RF amplitude to ' + str(self.increase) + ' - Last Power Reading was ' + str(max(self.latest_values['LLRF_CAVITY_FWD_PWR'])/1e6) + 'MW'
