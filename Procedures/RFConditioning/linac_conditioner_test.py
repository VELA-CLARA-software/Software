...
class linac_condition(QObject):
    ...
    def __init__(self, parent = None):
        ...
        self.log_file = "breakdowns_test.txt"
        self.rev_trace_log = "breakdown_trace_test.txt"
    ...
    def writetrace(self, filename=self.rev_trace_log, trace=self.latest_values['LLRF_CAVITY_REV_POWER']):
        with open(,"a") as logfile:
                logfile.write( str(time.time()) + '\t' + str(trace))
    ...
    def check_IMG_change_is_small(self, img_signal, rf_amp_signal, rf_pulse_width_signal):
        ...
        if self.delta_IMG > self.pv_monitor_not_alarm_values[img_signal]:
            ...
            self.monitor_Signal.emit(['dont_panic_text', "Vac Spike Detected!"])
