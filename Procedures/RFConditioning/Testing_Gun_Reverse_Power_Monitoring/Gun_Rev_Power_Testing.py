...
class RFTest(QMainWindow):
    ...
    def runAnalysis(self):
        ...
        if self.max_fwd_value > 1.5 and max(self.new_fwd_value) > 1e6:
            print 'Breakdown on the FWD power? Max = ', self.max_fwd_value
        if self.max_rev_value > 1.5 and max(self.new_fwd_value) > 1e6:
    ...
    def updatePlots(self):
        ...
        if len(self.timevector) == len(self.all_fwd_value):
        ...
        if len(cuttimevector) == len(self.mean_fwd_value):
