...
""" LOUISE: Upper limit, LLRF interlock"""
...
class linac_condition(QObject):
	...
	def check_LLRF_fwd_power_for_drop(self, fwd_signal,amp_signal):
		...
		if self.latest_values[fwd_signal] is not None:
			...
			if not self.incooldown and not self.rf_dropped and sum_fwd_power < self.pv_monitor_not_alarm_values[fwd_signal]*self.fwd_pwr_mean:
	...
	def can_increase_rf(self):
        if self.latest_values['RF_AMPLITUDE'] < self.rf_upper_limit:
	...
	def main_loop(self):
		...
		if self.doConditioning:
			...
			self.check_LLRF_fwd_power_for_drop('LLRF_CAVITY_FWD_PWR', 'RF_AMPLITUDE')
