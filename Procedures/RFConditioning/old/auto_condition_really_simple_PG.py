...
'cavity_fwd_power' : epics.PV('CLA-GUNS-LRF-CTRL-01:ad1:ch3:power_remote.POWER'),
}
...
delta_RF = 0; #RF step increment
wait_time = 1; #Time between RF step increments (in seconds)
drop_RF = 0; # RF step drop if spike
drop_RF_when_permit_lost = 0; # RF step drop if RF permit lost 
...
log_file = "llrf_amp_kly_fwd_pwr_log_DEV.txt"	
...
def log( lrf_index_start, llrf_index_stop):
	kly_power_data = pv_dictionary['kly_power_pv' ].get()
	cav_power_data = pv_dictionary['cavity_fwd_power' ].get()
	kly_power_cut   = kly_power_data[llrf_index_start:llrf_index_stop]
	cav_power_data = kly_power_data[llrf_index_start:llrf_index_stop]
	kly_power = numpy.mean( kly_power_cut )
	cav_power = numpy.mean( cav_power_data )
	message =  currenttime() +  ' RF to ' + str( value )	+ ' kly_fwd_power =  ' + str(kly_power) + ' cav_fwd_power =  ' + str(cav_power)
...
get_LLRF_power_trace_index_at_time_t
last_pulse_length = 0		
# return the index (number of points) less than t form the LLRF time 
def get_LLRF_power_trace_index_at_time_t( t ):
	# the LLRF times	
...
while True:
	...
	if can_increase_rf():
		...
		log( llrf_time_start, llrf_index_stop)
