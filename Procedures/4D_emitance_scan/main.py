...
log_file = 'emittance_log.txt'
num_beam_on_images = 10
num_beam_off_images = 1
...
should_degauss_in_main_loop = False
...
def start_up(magnets, screens, log_file ):
    ...
    global wcm_id
    wcm_id = start_WCM_bufffer()
    message('start_up: WCM = ' + str(get_wcm_value()),log_file=log_file)
    ...
    wait_for_screen_in(screens=screens)
...
def get_quad_1_2_on_data( screens, magnets, magnet_values, log_file ):
    ...
    if wait_for_screen_in(screens=screens):
...
def main_loop(magnets, values, log_file):
    ...
    message('main_loop: Passed values  = ' + ",".join(values), log_file=log_file)
    global num_beam_on_images
    global should_degauss_in_main_loop
    global num_beam_off_images
    for setting in values:
        vals = setting[:-2]
        message('main_loop: Setting Magnets to = ' + ','.join(str(e) for e in vals),
                log_file=log_file)
...
main_loop(magnets = magnets, values = mag_values, log_file = mag_values)
...
set_magnets_and_wait(mag_names= sm, mag_values = smv, tolerance= 0.01, timeout = 100)
# wait for screens in
...
start_up(mag_names, all_screen_names )
