...
class camdaq(hardware_base):
    ...
    def __init__(self, mode=MACHINE_MODE.PHYSICAL, area=MACHINE_AREA.CLARA_PH1):
        ...
        self._my_name = camdaq._my_name
        self.init_camdaq()
        self.get_controller_func(self.init.physical_CLARA_Camera_DAQ_Controller)
        self.get_names(self.controller.getCameraNames)
        print(camdaq._my_name + ' has controller, testing procedure')
        self.test_procedure()
        if self.procedure_good:
            print(camdaq._my_name + ' Procedure Good')
        else:
    ...
    def init_camdaq(self):
        camdaq.init = daq.init()
        camdaq.init.setQuiet()

    def test_procedure(self):
        'assumes [names,state] for each iteration'
        if self.parse_procedure(self.config.camdaq_data):
    ...
    def next_step(self, num):
        self.iteration = self.get_it_names_values(self.config.camdaq_data[num])
        print self.iteration
        for it in self.iteration:
            self.activate(name=it[0],value=it[1])

        print(camdaq._my_name + ' iteration applied')
