...
#HMCC import SimulationFramework.Modules.read_twiss_file as read_twiss_file
import  SimulationFramework.Modules.Twiss as read_twiss_file
...
class GetDataFromSimFrame(object):
	...
	def setSimulationDictDefaults(self, datadict, vc_object=None, wcm_object=None,
		...
		if start_lattice=='Generator':
			...
			datadict['generator'].update({'sigma_x': {}})
			datadict['generator']['sigma_x'].update({'type': 'generator'})
			datadict['generator']['sigma_x'].update({'catap_type': 'pil'})
			datadict['generator']['sigma_x'].update({'value': 0.25})
			datadict['generator'].update({'sigma_y': {}})
			datadict['generator']['sigma_y'].update({'type': 'generator'})
			datadict['generator']['sigma_y'].update({'catap_type': 'pil'})
			datadict['generator']['sigma_y'].update({'value': 0.25})
			datadict['generator'].update({'offset_x': {}})
			datadict['generator']['offset_x'].update({'type': 'generator'})
			datadict['generator']['offset_x'].update({'catap_type': 'pil'})
			datadict['generator']['offset_x'].update({'value': 0.25})
			datadict['generator'].update({'offset_y': {}})
			datadict['generator']['offset_y'].update({'type': 'generator'})
			datadict['generator']['offset_y'].update({'catap_type': 'pil'})
			datadict['generator']['offset_y'].update({'value': 0.25})
			...
			if vc_object is not None:
				datadict['generator']['sigma_x'].update({'value': vc_object['x_mm_sig']})
				datadict['generator']['sigma_y'].update({'value': vc_object['y_mm_sig']})
				datadict['generator']['spot_size'].update({'value': float(numpy.mean([vc_object['x_mm_sig'],
																				vc_object['y_mm_sig']]))})
				datadict['generator']['offset_x'].update({'value': vc_object['x_mm']})
				datadict['generator']['offset_y'].update({'value': vc_object['y_mm']})
	...
	def getAllBeamFiles(self, directory):
		for file in os.listdir(directory):
			if file.endswith(".hdf5") and not file.startswith('Beam_') and not file.startswith('Twiss_'):
				...
				if "SCR" in file:
					...
					self.beamdata.update({'type':"aperture"})
				else:
				...
				self.beamdata['x'].update({'var': numpy.var(getattr(self.beam, 'x'))})
				...
				self.beamdata['y'].update({'var': numpy.var(getattr(self.beam, 'y'))})
				self.beamdata.update({'x_y':{}})
				self.beamdata['x_y'].update({'mean':float(numpy.mean(numpy.multiply(getattr(self.beam, 'x'), getattr(self.beam, 'y'))))})
				...
				self.beamdata['px'].update({'var': numpy.var(getattr(self.beam, 'cpx'))})
				...
				self.beamdata['py'].update({'var': numpy.var(getattr(self.beam, 'cpy'))})
				...
				self.beamdata['pz'].update({'var': numpy.var(getattr(self.beam, 'cpz'))})
