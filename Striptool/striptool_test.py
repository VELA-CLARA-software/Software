...
def createRandomSignal():
    signalValue = np.sin(5*2*np.pi*time.time())+np.sin(10*2*np.pi*time.time())+np.random.normal()
    return signalValue
...
def main():
    ...
    sp.addSignal(name='signal1',pen='r', timer=1.0/100.0, function=createRandomSignal)
    sp.addSignal(name='signal2',pen='g', timer=1.0/10.0, function=createRandomSignal)
