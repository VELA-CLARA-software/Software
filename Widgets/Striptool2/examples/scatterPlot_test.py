...
class scatterPlot_Demo(QMainWindow):
    ...
    def createRandomSignal(self, offset=0):
        signalValue = np.sin(2 * 2 * np.pi * time.time() + 0.05) + np.sin(
            1.384 * 2 * np.pi * time.time() - 0.1) + 0.5 * np.random.normal()
        return signalValue + offset

    def createRandomSignal2(self, offset=0):
        signalValue = -1 * (np.sin(2 * 12 * np.pi * time.time() + 0.05) + np.sin(
            0.2563 * 2 * np.pi * time.time() - 0.1)) + 0.5 * np.random.normal()
        return signalValue + offset
