...
class stripLegend(pg.TreeWidget):
    ...
    verticalMaxChanged = pyqtSignal('PyQt_PyObject','PyQt_PyObject')
    verticalMinChanged = pyqtSignal('PyQt_PyObject','PyQt_PyObject')
    ...
    def addLegendItem(self, name):
        ...
        ''' Vertical Scale'''
        verticalmax = QLineEdit()
        verticalmax.setValidator(QDoubleValidator())
        verticalmax.setText(str(self.records[name]['VerticalMax']))
        verticalmax.editingFinished.connect(lambda: self.changeVerticalMax(name, verticalmax))
        self.addTreeWidget(parentTreeWidget, name, "Signal Max", verticalmax)
        ''' Vertical Offset'''
        verticalmin = QLineEdit()
        verticalmin.setValidator(QDoubleValidator())
        verticalmin.setText(str(self.records[name]['VerticalMin']))
        verticalmin.editingFinished.connect(lambda: self.changeVerticalMin(name, verticalmin))
        self.addTreeWidget(parentTreeWidget, name, "Signal Min", verticalmin)
        ''' Signal Centered'''
    ...
    def changeVerticalMinMax(self, vb, minmax):
        self.verticalmin.setText(str(minmax[0]))
        self.verticalmax.setText(str(minmax[1]))

    def changeVerticalMax(self, name, verticalmax):
        self.records[name]['VerticalMax'] = verticalmax.text()
        self.verticalMaxChanged.emit(name,float(verticalmax.text()))

    def changeVerticalMin(self, name, verticalmin):
        self.records[name]['VerticalMin'] = verticalmin.text()
        self.verticalMinChanged.emit(name,float(verticalmin.text()))
