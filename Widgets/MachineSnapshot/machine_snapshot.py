import h5py, json, numpy, math, os, sys
...
class MachineSnapshot(object):
    def __init__(self, MAG_Ctrl=None, BPM_Ctrl=None, CHG_Ctrl=None,
                 ...
                 MACHINE_MODE=vce.MACHINE_MODE.PHYSICAL, MACHINE_AREA=vce.MACHINE_AREA.CLARA_2_BA1_BA2, bufferSize=10,
                 messages=False):
        """Set up for a machine snapshot, loading all the relevant controllers."""
        ...
        self.L01_MOD_Ctrl = L01_MOD_Ctrl
    ...
    def checkModController(self):
        ...
        if self.L01_MOD_Ctrl is not None:
            self.l01_mod_ctrl = self.L01_MOD_Ctrl
        else:
    ...
    def getmagnetdata(self):
        ...
        if self.L01_Momentum == None:
            for i in self.dip_names:
                if i == "S02-DIP01":
                    ...
                    self.coeffs = numpy.append(self.dip_objs[i].fieldIntegralCoefficients[:-1],
                                               self.dip_objs[i].fieldIntegralCoefficients[-1])
                    self.int_strengthdip['L01'] = numpy.polyval(self.coeffs, abs(self.current))
                    self.angle = 45
                    self.momentum['L01'] = 0.001 * SPEED_OF_LIGHT * self.int_strengthdip['L01'] / numpy.radians(
                        self.angle)
        else:
        ...
        for i in self.mag_names:
            ...
            self.coeffs = numpy.append(self.mag_objs[i].fieldIntegralCoefficients[:-1],
                                       self.mag_objs[i].fieldIntegralCoefficients[-1])
            ...
            # self.mag_data[i]['deflection'] = self.effect  # deflection in mrad
                ...
                self.mag_data[i]['cathode_field'] = numpy.dot(self.k_coeffs,
                                                              [self.y, self.y ** 2, self.y ** 3, self.x,
                                                               self.x * self.y, self.x * self.y ** 2, self.x ** 2,
                                                               self.x ** 2 * self.y, self.x ** 2 * self.y ** 2,
                                                               self.x ** 2 * self.y ** 3, self.x ** 3,
                                                               self.x ** 3 * self.y])
                ...
                self.p = self.mom
                self.k_coeffs = numpy.array(self.mag_objs[i].fieldIntegralCoefficients[:-4])
                self.mag_data[i]['larmor_angle'] = numpy.dot(self.k_coeffs,
                                                             [self.I, self.p * self.I, self.p ** 2 * self.I,
                                                              self.p ** 3 * self.I, self.p ** 4 * self.I])
        ...
        for i in self.bpm_names:
            ...
            if self.bpm_ctrl.getBPMStatusStr(i) == "GOOD":
                while self.bpm_ctrl.isDataBufferNotFull(i):
                    ...
                    extra_time = 0
                    if self.timesleep > (self.timenow + (self.bufferSize / 10.0) + extra_time):
                        break
    ...
    def getllrfdata(self, llrfobj, llrfctrl, data_dict):
        ...
        if self.llrfobj.name == "CLA-LRRG-LLRF":
            self.all_traces_to_monitor = ["KLYSTRON_FORWARD_POWER", "KLYSTRON_FORWARD_PHASE", "KLYSTRON_REVERSE_PHASE",
                                          "KLYSTRON_REVERSE_POWER", "LRRG_CAVITY_REVERSE_PHASE",
                                          "LRRG_CAVITY_FORWARD_PHASE", "LRRG_CAVITY_REVERSE_POWER",
                                          "LRRG_CAVITY_FORWARD_POWER"]
    ...
    def getdata(self):
        ...
        self.chargedata = self.getchargedata()
        print(datetime.datetime.now(), 'Charge')
        self.magnetdata = self.getmagnetdata()
        # self.gunmoddata = self.getgunmodulatordata()
        # self.l01moddata = self.getl01modulatordata()
        print(datetime.datetime.now(), 'Magnets')
        self.screendata = self.getscreendata()
        print(datetime.datetime.now(), 'Screens')
        self.pildata = self.getpildata()
        print(datetime.datetime.now(), 'PIL')
        self.cameradata = self.getcameradata()
        print(datetime.datetime.now(), 'Camera')
        self.gundata = self.getgundata()
        print(datetime.datetime.now(), 'Gun')
        self.l01data = self.getl01data()
        print(datetime.datetime.now(), 'L01')
    ...
    def writetohdf5(self, filename=None, directory=None, data=None):
        """Write data to a HDF5 file. If a fully-qualified filename is not suppplied, create one in the work folder."""
        self.data = data
        self.filename = self.setfilename(filename, directory) if filename is None else filename
        self.fullname = self.filename if self.filename.lower().endswith('.hdf5') else self.filename + ".hdf5"
        print(self.fullname)
    ...
    def getdirectory(self):
        ...
        self.directory = work_folder + now.strftime(r'\%Y\%#m\%#d')  # omit leading zeros from month and day
        if not os.path.isdir(self.directory):
            os.mkdir(self.directory)
...
if __name__ == "__main__":
    data = MachineSnapshot(MAG_Ctrl=None, BPM_Ctrl=None, CHG_Ctrl=None,
                           ...
                           PIL_Ctrl=None, MACHINE_MODE=vce.MACHINE_MODE.PHYSICAL,
                           MACHINE_AREA=vce.MACHINE_AREA.CLARA_2_BA1_BA2, bufferSize=10, messages=False)
    data.getData()
    data.writetohdf5()
    print "fin"
    time.sleep(5)
