...
def do_plotting(subplot, vcx, vcy, charge, cmin, cmax, title):
    ...
    cbar = plt.colorbar(label = 'Charge [pC]')#im, cax=cax, orientation='horizontal')
...
do_plotting(1, vcx1, vcy1, charge1, 0, maxc, title1)
...
do_plotting(2, vcx2, vcy2, charge2, 0, maxc, title2)

charge_diff = charge2 - charge1
do_plotting(3, vcx2, vcy2, charge_diff, np.min(charge_diff), np.max(charge_diff), 'Difference')
