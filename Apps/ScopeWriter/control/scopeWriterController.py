...
class scopeWriterController(QObject):
	...
	def startLogging(self):
		...
		while self.started == True:
			...
			for channel in self.layoutWidgets( self.view.channelsVBox ):
				...
				self.diagnosticType = str( channel.itemAt(3).itemAt(1).widget().currentText() )
				self.filterType = str( channel.itemAt(4).itemAt(1).widget().currentText() )
				self.filterInterval = str( channel.itemAt(5).itemAt(1).widget().toPlainText() )
				if self.measurementType == "Area":
					self.signal = channel.itemAt(6)
					...
					self.readTracesAndWriteAreaToEPICS(self.scopeName, self.traceChannelPV, self.signalStart, self.signalEnd, self.epicsPVName, self.filterType, self.filterInterval, self.diagnosticType)
				elif self.measurementType == "Max":
					self.readTracesAndWriteMaxToEPICS(self.scopeName, self.traceChannelPV, self.epicsPVName, self.filterType, self.filterInterval, self.diagnosticType)
				elif self.measurementType == "Min":
					self.readTracesAndWriteMinToEPICS(self.scopeName, self.traceChannelPV, self.epicsPVName, self.filterType, self.filterInterval, self.diagnosticType)
				elif self.measurementType == "Peak-to-Peak":
					self.readTracesAndWriteP2PToEPICS(self.scopeName, self.traceChannelPV, self.epicsPVName, self.filterType, self.filterInterval, self.diagnosticType)
				else:
	...
	def readTracesAndWriteAreaToEPICS( self, scope_names, channel_name, area_start, area_end, epics_channel, filter_type, filter_interval, diagnostic_type ):
		...
		self.diagnostic_calibration = self.getDiagnosticCalibration( self.diagnostic_type )
		# print self.mean_area
		epics.caput( self.epics_channel, abs(self.mean_area * self.diagnostic_calibration) )

	def readTracesAndWriteMaxToEPICS( self, scope_names, channel_name, epics_channel, filter_type, filter_interval, diagnostic_type ):
		self.wvf_name = scope_names[0]
		self.scop_name = scope_names[1]
		self.channel_name = channel_name
		self.epics_channel = epics_channel
		...
		self.mean_max = numpy.mean( self.max )
		self.diagnostic_calibration = self.getDiagnosticCalibration( self.diagnostic_type )

		epics.caput( self.epics_channel, abs(self.mean_max * self.diagnostic_calibration) )
		time.sleep(0.1)

	def readTracesAndWriteMinToEPICS( self, scope_names, channel_name, epics_channel, filter_type, filter_interval, diagnostic_type ):
		self.wvf_name = scope_names[0]
		self.scop_name = scope_names[1]
		self.channel_name = channel_name
		self.epics_channel = epics_channel
		...
		epics.caput( self.epics_channel, abs(self.mean_min * self.diagnostic_calibration) )
		time.sleep(0.1)

	def readTracesAndWriteP2PToEPICS( self, scope_names, channel_name, epics_channel, filter_type, filter_interval, diagnostic_type ):
		self.wvf_name = scope_names[0]
		self.scop_name = scope_names[1]
		self.channel_name = channel_name
		self.epics_channel = epics_channel
		...
		epics.caput( self.epics_channel, abs(self.mean_p2p * self.diagnostic_calibration) )
		time.sleep(0.1)
