...
//  Description: walk through of the online analysis procedure.

                 Image Analysis History (notes in .\docs):
                 ...
                 class in TP CameraIA module in github.

                 (2015 djs played with generalised extensions to fitting bivariate normal
                 distributions: Characterising 2D Beam Distributions Generalised Normal
                 Distributions and Copulas)
"""
from __future__ import division  # force python division to use floats !!!
import h5py
import os
import numpy as np
...
def get_first_hdf5_image(path, filename):
    """
    gets the FIRST image from the passed file, we have to hardcode in the 'Capture000001'
...
"""
    to make the math easier we can choose smart values ...
...
"""
    now we have the "optimum" sca;ing factor for the mask, we should use that instead
"""
...
"""
    Now Lets' calculate the moments of the scaled projections
...
# it seems we need to make a new ellipse/rectangle each time we want to inlcude one
full_image_mask_ellipse_art = Ellipse((mask_horizontal_center, mask_vertical_center),
                                      width=mask_horizontal_radius * 2,
                                      height=mask_vertical_radius * 2, edgecolor='white',
...
cut_image_mask_ellipse_art_2 = Ellipse((mask_horizontal_radius, mask_vertical_radius),
                                       ...
                                       height=mask_vertical_radius * 2, edgecolor='white',
                                       facecolor='none', linewidth=1)
# of course rectangles are defined differently,
# https://matplotlib.org/3.1.1/api/_as_gen/matplotlib.patches.Rectangle.html
mask_boundingbox_art = Rectangle(
    (mask_boundingbox_horizontal_start, mask_boundingbox_vertical_start),
    width=mask_horizontal_radius * 2, height=mask_vertical_radius * 2, edgecolor='red',
    facecolor='none', linewidth=1)
