...
class svdNoiseRemover():
    ...
    def calculateDarkCharge(self, wcm="clara", mode=1):
        ...
        return np.mean(self.charge)*1e12,np.std(self.charge)*1e12


    def writesvdresults(self, mydir, time):
        np.savetxt(mydir+'Chargedata'+str(time)+'.txt',self.charge.transpose())
