...
import math as m
import random as r
from numpy import zeros, polyval, roots, arange, argmin
...
class Functions(QObject):
    ...
    def align2(self,hctrl,hcor, bctrl, bpm, tol, N=10):
        print 'In align2, in align2..', bpm, type(bpm), len(bpm)
        COR = hctrl.getMagObjConstRef(hcor)                                        #create a reference to the corrector
        x1= self.getXBPM(bctrl, bpm, N)                                            #get the x position on the BPM
        I1 = COR.siWithPol
        print 'x1 = ', x1, 'I1 = ', I1
        ...
        # time.sleep(1)
        # print 'x1 = ', x1, 'I1 = ', I1
                                                      #x1,x2,I1,I2 are point to determine a straight linear relationship (I=mx+c)
        x2=x1
        I2=0
        if (COR.riWithPol>0.0):                                                    #determine intial step
            initialStep = -0.1
    ...
    def minimizeBeta(self,qctrl,quad,sctrl,screen,init_step,N=1):
        ...
        #     print sX_scan
        #     sX_approxmin = xrange[argmin(sX_scan)]
        #     print sX_approxmin
        # else:
        #     print 'Rough scan in Y'
        #     sY_scan = []
        #     yrange = arange(-50,50,10)
        #     for y in yrange:
        #         qctrl.setSI(quad, y)
        #         self.simulate.run()
        #         time.sleep(1)
        ...
        #     print sY_scan
        #     sY_approxmin = yrange[argmin(sY_scan)]
        #     print sY_approxmin
    ...
    def findDispersion(self,dctrl,dipole,sctrl,screen,centering_I,points,leveloff_threshold,N=1):
        currents = zeros(points)
        positions = zeros(points)
        dCurrents=[]
        dPositions=[]
        fCurrents=[]
        fPositions=[]
    ...
    def calcMomSpread(self,dctrl,dipole, Is, I):
        ...
        mom1= (400.0033/D.magneticLength)*(polyval(D.fieldIntegralCoefficients, abs(I-Is))*physics.c*180)/(45*physics.pi*1000000000)
        mom2= (400.0033/D.magneticLength)*(polyval(D.fieldIntegralCoefficients, abs(Is+I))*physics.c*180)/(45*physics.pi*1000000000)
        print(mom1-mom2)/2
        return abs(mom1-mom2)/2

    def calcMom(self, dctrl,dipole, I):
        D = dctrl.getMagObjConstRef(dipole)
        return (400.0033/D.magneticLength)*(polyval(D.fieldIntegralCoefficients, abs(I))*physics.c*180)/(45*physics.pi*1000000000)
...
class MinimiseBetaClass(threadObject):
    ...
    def minimizeBeta2(self):
        ...
        if(minimisingInX):
            ...
            sX_scan2 = []
            xrange2 = arange(sX_approxmin-10,sX_approxmin+10,1)
            for x in xrange2:
                self.qctrl.setSI(self.quad, x)
                #self.simulate.run()
                time.sleep(1)
            ...
            print sX_scan2
            sX_min = xrange2[argmin(sX_scan2)]
            print sX_min
            ...
            sY_scan2 = []
            yrange2 = arange(sY_approxmin-10,sY_approxmin+10,2)
            for y in yrange2:
                self.qctrl.setSI(self.quad, y)
                #self.simulate.run()
                time.sleep(1)
            ...
            print sY_scan2
            sY_min = yrange2[argmin(sY_scan2)]
            print sY_min
