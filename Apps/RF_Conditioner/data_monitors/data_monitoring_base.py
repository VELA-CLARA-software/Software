...
class data_monitoring_base(object):
	...
	def start_monitors(self,
		...
		if bool(vac_param):
			self.start_vac_monitor(vac_param)
		if bool(DC_param):
			self.start_DC_monitor(DC_param)
		if bool(llrf_param):
	...
	def start_vac_monitor(self, vac_param):
	...
	def start_DC_monitor(self, DC_param):
		# NOT HAPPY ABOUT HARDCODED STRINGS...
		if self.connectPV(self.DC_id, DC_param.get('VAC_PV')):  # MAGIC_STRING
			DC_param[self.vac_id] = self.gen_mon_keys[self.DC_id]
			# Vacuum monitoring requires a vacuum PV
