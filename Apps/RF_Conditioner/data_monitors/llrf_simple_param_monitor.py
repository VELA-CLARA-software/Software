...
class llrf_simple_param_monitor(monitor):
    ...
    def __init__(self):
        ...
        for trace in monitor.config.llrf_config['MEAN_TRACES']:
            trace2 = monitor.llrf_control.fullCavityTraceName(trace)
            ...
            self.old_mean_values[monitor.llrf_control.fullCavityTraceName(trace)]  = 0
    ...
    def update_value(self):
        ...
        if monitor.data.values[dat.phase_mask_by_power_trace_1_set]:
            monitor.data.values[dat.phase_end_mask_by_power_trace_1_time] = monitor.llrf_control.getMaskInfiniteEndByPowerTime(
                self.phase_trace_1 )
        if monitor.data.values[dat.phase_mask_by_power_trace_2_set]:
            monitor.data.values[dat.phase_end_mask_by_power_trace_2_time] = monitor.llrf_control.getMaskInfiniteEndByPowerTime(
                self.phase_trace_1 )

    def get_mean_power(self,key,trace):
        v = monitor.llrfObj[0].trace_data[trace].traces[monitor.llrfObj[0].trace_data[trace].latest_trace_index].mean
