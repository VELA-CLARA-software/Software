...
class main_controller(controller_base):
				...
				if controller_base.llrf_handler.mask_set == False:
					...
					if controller_base.data.reached_min_pulse_count_for_this_step(): # check this number in the look up
						if self.gui.can_ramp:
							if controller_base.data[dat.breakdown_rate_hi]:
								if i > 0:
									self.logger.message('MAIN LOOP allgood, but breakdown rate too high ' + str(controller_base.data[dat.breakdown_rate]))
									i = 1
							else:
								self.ramp_up()
								i = 0
	...
	def continue_ramp(self):
		...
		controller_base.llrf_handler.set_amp(self.data.amp_sp_history[-1])
