...
class llrf_monitor(monitor):
    ...
    def update_amp_vs_kfpow_running_stat(self):
        ...
        self.data.amp_vs_kfpow_running_stat[self.values[self.data.amp_sp]] = self.llrf_control.getKlyFwdPwrRSState(int(self.values[
                                                                                                                           self.data.amp_sp]))  #
        # TODO THIS HAS TO BE AN INT IN THE C++ ??? WTF  # TODO need c++ to return zeros if the key does not exist  # print("update
        #  amp_vs_kfpow_running_stat with amp_sp = {}".format(self.values[  #  self.data.amp_sp]))
