...
class config(object):
    ...
    def raw_config(self, keywords):
        '''
        This is the same as 'split_raw_config' function, just renamed for clarity as it is not
        actually splitting the raw config
        '''
        return_dictionary = {}
    ...
    def sanity_checks(self):
        ...
        #START#########################################################################################
        #TODO AJG: recast 'num_keywords_found' to num_keywords_found_RAW and it reads length of 'config.raw_config_data'
        # ...probably not necessary

        num_keywords_found_RAW = len(config.raw_config_data)
        print('num_keywords_found_RAW = ', num_keywords_found_RAW)
        #END###########################################################################################
    ...
    #TODO AJG: Create Mega config dictionary
    # Need to pass around 'keywords_RAW' now NOT 'all_config_keywords' or individual keywords
    # maybe with the exception of gmonitor_keywords = [GMON_SUFFIX] ???
    ...
    keywords_RAW = [WHEN_VAC_HI_DISABLE_RAMP,VAC_NUM_SAMPLES_TO_AVERAGE, VAC_SPIKE_DECAY_LEVEL,
                    ...
                    VAC_SHOULD_DROP_AMP, VAC_SPIKE_DROP_AMP,  # VAC_MAX_AMP_DROP,
                    VAC_SPIKE_DELTA, VAC_CHECK_TIME, VAC_DECAY_MODE, VAC_MAX_LEVEL, VAC_PV,
                    MINIMUM_COOLDOWN_TIME,DC_PV, DC_DECAY_MODE, DC_SPIKE_DECAY_LEVEL, DC_SPIKE_DELTA,
                    DC_SPIKE_DROP_AMP,
                    DC_NUM_SAMPLES_TO_AVERAGE, DC_SPIKE_DECAY_TIME, DC_CHECK_TIME,
                    DC_SHOULD_DROP_AMP,TEXT_LOG_FILENAME, LOG_DIRECTORY, BINARY_DATA_LOG_FILENAME,
                    OUTSIDE_MASK_FORWARD_FILENAME,
                    OUTSIDE_MASK_REVERSE_FILENAME, OUTSIDE_MASK_PROBE_FILENAME,
                    PULSE_COUNT_BREAKDOWN_LOG_FILENAME, PULSE_COUNT_BREAKDOWN_LOG_FILENAME,
                    KFPOW_AMPSP_RUNNING_STATS_LOG_FILENAME, BINARY_DATA_LOG_TIME, AMP_PWR_LOG_TIME,
                    VAC_VALVE, VAC_VALVE_AREA, VAC_VALVE_CHECK_TIME, KEEP_VALVE_OPEN, CAVITY_TEMPERATURE_PV,
                    CAVITY_TEMPERATURE_CHECK_TIME, CAVITY_TEMPERATURE_PV_COUNT, WATER_TEMPERATURE_PV,
                    WATER_TEMPERATURE_CHECK_TIME, WATER_TEMPERATURE_PV_COUNT, RF_STRUCTURE,
                    DEFAULT_RF_INCREASE_LEVEL, RF_REPETITION_RATE,
                    RF_REPETITION_RATE_ERROR, BREAKDOWN_RATE_AIM, LLRF_CHECK_TIME,
                    # LOW_POWER_INCREASE,
                    # LOW_POWER_INCREASE_RATE_LIMIT,
                    NUMBER_OF_PULSES_IN_BREAKDOWN_HISTORY, VAC_SPIKE_TRACES_TO_SAVE,
                    ...
                    CPPOW_MEAN_START, CPPOW_MEAN_END,
                    # NORMAL_POWER_INCREASE
                    MAGNET_MACHINE_AREA, SOL_NAMES, SOL_CHECK_TIME, SHOULD_SOL_WOBBLE,
                    SOL_WOBBLE_HI, SOL_WOBBLE_LO, SOL_COUNT,
                    SOL_WOBBLE_OSCILLATION_TIME, SOL_WOBBLE_OSCILLATION_POINTS,
                    CFPOW_AUTO_SET, CRPOW_AUTO_SET, CPPOW_AUTO_SET, KFPOW_AUTO_SET,
                    KRPOW_AUTO_SET, CFPHA_AUTO_SET, CRPHA_AUTO_SET, CPPHA_AUTO_SET,
                    KFPHA_AUTO_SET, KRPHA_AUTO_SET, CFPOW_DROP_AMP, CRPOW_DROP_AMP,
                    CPPOW_DROP_AMP, KFPOW_DROP_AMP, KRPOW_DROP_AMP, CFPHA_DROP_AMP,
                    CRPHA_DROP_AMP, CPPHA_DROP_AMP, KFPHA_DROP_AMP, KRPHA_DROP_AMP,
