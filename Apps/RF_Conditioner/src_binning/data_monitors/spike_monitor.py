    ...
    def check_for_spike(self):
        ...
        if self._mean_level != None:
            if self._latest_value > self.spike_delta + self._mean_level:
                ...
                # OLD dump_data position (see below notes for more info)
                # self.dump_data()
                ...
                # When operating at 100Hz we ALWAYS saw zero power traces in teh data_dump
                # AT 10 HZ we are seeing NO zero power traces. This is probably because the data has been
                # dumped before the next pulse has been sent out by the RF system
                # dump_data
                self.dump_data()
