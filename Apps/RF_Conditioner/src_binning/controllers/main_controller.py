...
class main_controller(controller_base):
    ...
    def ramp_up(self):
        ...
        if controller_base.llrf_handler.set_amp(new_amp):
            ...
            # For now, disable ramping and hope for the best
    ...
    def ramp_down(self):
        ...
        # HACK WATCH OUT!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        # HACK WATCH OUT!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        if controller_base.data.values[dat.last_106_bd_count] > 30000:  # MAGIC_NUMBER
        ...
        elif controller_base.data.values[dat.last_106_bd_count] > 5000:  # MAGIC_NUMBER
            ...
            # ADDING IN ONLY RAMP DOWN ON EVERY 10 MINS
            if controller_base.data.values[dat.breakdown_count] % 10  ==0:  #MAGIC NUMBER , only ramp down every 10 events
                next_step = controller_base.data.values[dat.next_sp_decrease]
                ...
                next_step = controller_base.data.values[dat.last_sp_above_100]
                controller_base.llrf_handler.set_amp(next_step)  # MAGIC_NUMBER
                self.logger.message(
                    'event return FINISHED, we went from ' + str(setpoint_before_ramp) + ' to ' + str(
                        next_step), True)
