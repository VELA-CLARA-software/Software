...
from viewSource.Ui_mainView3 import Ui_mainView
...
############
        self.x_proj_plot_2 = self.x_proj_2.addPlot(lockAspect=True)
        self.x_2_curve1 = self.x_proj_plot_2.plot(pen=2)
        self.x_2_curve2 = self.x_proj_plot_2.plot(pen=1)
        #self.x_proj_plot_2.setRange(xRange=[-300,300])

        #self.x_proj_plot_2_lo_line = self.x_proj_plot_2.addLine(x=self.data.values[data.fwhm_lo])
        #self.x_proj_plot_2_hi_line = self.x_proj_plot_2.addLine(x=self.data.values[data.fwhm_hi])
    ...
    def update_gui(self):
        ...
        # projection centred on mean
        #print 'array length', len(view.data.values[data.x_proj])
        # centred on max
        #x_coords = range(len(view.data.values[data.x_proj]))-view.data.values[data.x_proj_max_index]
        # centred on mean
        #x_offset = self.mean_index(view.data.values[data.x_proj_2])
        #x_coords = array(range(len(view.data.values[data.x_proj_2])))#-x_offset
        length = len(self.data.values[data.x_proj_2])
        #print '\n\nlength = ', length
        halflength = int(round(length/2))
        x_coords = array(range(length))-halflength
        self.x_2_curve1.setData(x = x_coords, y = view.data.values[data.x_proj_2])
        ...
        if view.data.values[data.has_ref]:
            if view.data.values[data.ref_plotted]:
                ...
                self.x_2_curve2.setData(x = x_coords, y = view.data.values[data.x_ref_2])
                self.y_curve2.setData(y =y_coords,
                                  x = view.data.values[data.y_ref])
                self.x_curve2.show()
                self.x_2_curve2.show()
                self.y_curve2.show()
                view.data.values[data.ref_plotted] = True
        else:
            self.x_curve2.hide()
            self.x_2_curve2.hide()
            self.y_curve2.hide()
        ...
        #self.x_proj_plot_2_lo_line.setValue( v = self.data.values[data.fwhm_lo])#-x_offset)
        #self.x_proj_plot_2_hi_line.setValue( v = self.data.values[data.fwhm_hi])#-x_offset)
        #self.x_proj_plot_lo_line.setValue( v = self.data.values[data.fwhm_lo_2])
        #self.x_proj_plot_hi_line.setValue( v = self.data.values[data.fwhm_hi_2])
