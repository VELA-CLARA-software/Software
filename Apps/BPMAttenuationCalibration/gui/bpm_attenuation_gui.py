...
class bpm_attenuation_gui(QMainWindow, Ui_MainWindow, base):
	...
	def __init__(self,
		...
		self.calibrateButton.clicked.connect(self.handle_calibrate_attenuations_button)
	...
	def update_gui(self):
		...
		if self.data.values[dat.plots_done] and not self.data.values[dat.values_saved]:
			...
			self.newATTVals.setText("")
			self.newATTVals.setText("BPM name " + str(self.data.values[dat.bpm_name]) + "\n" +
	...
	def get_sa_start(self):
		self.set_sa_start_text = self.lowerATTBoundOutputWidget.toPlainText()
		if len(self.set_sa_start_text) > 0:
			self.data.values[dat.set_sa_start] = long(self.set_sa_start_text)
			self.set_sa_start_set = True
			return True
		else:
			self.set_sa_start_set = False
			return False

	def get_sa_end(self):
		self.set_sa_end_text = self.upperATTBoundOutputWidget.toPlainText()
		if len(self.set_sa_end_text) > 0:
			self.data.values[dat.set_sa_end] = long(self.set_sa_end_text)
			self.set_sa_end_set = True
			return True
		else:
			self.set_sa_end_set = False
			return False
	...
	def check_ready(self):
		if self.get_bpm_name() and self.get_sa_start() and self.get_sa_end() and self.get_num_shots():
			self.data.values[dat.all_values_set] = True
			if self.data.values[dat.scope_monitoring]:
				self.data.values[dat.ready_to_go] = True

	# button functions
	def handle_calibrate_attenuations_button(self):
		self.check_ready()
	...
	def plot_bpm_vs_sa(self):
		...
		# self.xplot.setData(x=self.data.values[dat.bpm_v11_v12_sum].keys(), y=self.data.values[dat.bpm_v11_v12_sum].values(), symbol='o', pen=mkPen('b', width=3))
		self.xplot.setData(range(self.data.values[dat.set_sa_start],self.data.values[dat.set_sa_end]),
						   self.data.values[dat.bpm_v11_v12_sum].values())
		# pyqtgraph.plot(x=range(self.data.values[dat.set_sa_start],self.data.values[dat.set_sa_end]), y=self.data.values[dat.bpm_v11_v12_sum].values(), symbol='o')
		self.vby = self.bpmyPlot.vb
		self.vby.setYRange(-2, 2)
		self.yplot = self.bpmyPlot.plot(pen=mkPen('b', width=3), symbol='o')
		# self.xplot.setData(x=self.data.values[dat.bpm_v11_v12_sum].keys(), y=self.data.values[dat.bpm_v11_v12_sum].values(), symbol='o', pen=mkPen('b', width=3))
		self.yplot.setData(range(self.data.values[dat.set_sa_start], self.data.values[dat.set_sa_end]),
