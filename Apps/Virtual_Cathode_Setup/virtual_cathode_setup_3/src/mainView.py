...
class mainView(QtGui.QMainWindow, Ui_mainView ):
    ...
    def autoSetLevel(self):
        self.spinBox_minLevel.setValue( self.data.values[self.data.min_level_rbv] )
        self.spinBox_maxLevel.setValue( self.data.values[self.data.max_level_rbv] )
        self.setLevel()
    ...
    def update_crosshair(self):
        ...
        xmin = self.data.values[self.data.x_pix] - self.data.values[self.data.sig_x_pix]
        xmax = self.data.values[self.data.x_pix] + self.data.values[self.data.sig_x_pix]
        y0 = self.data.values[self.data.y_pix]
        ymin = self.data.values[self.data.y_pix] - self.data.values[self.data.sig_y_pix]
        ymax = self.data.values[self.data.y_pix] + self.data.values[self.data.sig_y_pix]
    ...
    def update_read_roi(self):
        ...
        :return:
        '''
        self.read_roi.setPos(QtCore.QPoint(self.data.values[self.data.mask_x_rbv] -
                                           self.data.values[self.data.mask_x_rad_rbv],
                                           self.data.values[self.data.mask_y_rbv] -
                                           self.data.values[self.data.mask_y_rad_rbv]))
        self.read_roi.setSize(QtCore.QPoint(2 * self.data.values[self.data.mask_x_rad_rbv],
                                            2 * self.data.values[self.data.mask_y_rad_rbv]))
    ...
    def start_up(self):
        ...
        self.maskX_spinBox.setRange(0,self.data.values[self.data.xpix_full])
        self.maskY_spinBox.setRange(0,self.data.values[self.data.ypix_full])
        self.maskXRadius_spinBox.setRange(0,self.data.values[self.data.ypix_full])
        self.maskYRadius_spinBox.setRange(0,self.data.values[self.data.ypix_full])

        self.mirror_h_step_set_spinBox.setValue(self.data.values[self.data.H_step_read])
        self.mirror_v_step_set_spinBox.setValue(self.data.values[self.data.V_step_read])
        # update gui widget
        for key, value in self.widget_to_dataname.iteritems():
            if self.is_mask_read(key):
                if not_updated_read_roi:
    ...
    def set_user_mask_to_read_mask(self):
        x = self.data.values[self.data.mask_x_rbv] - self.data.values[
            self.data.mask_x_rad_rbv]
        y = self.data.values[self.data.mask_y_rbv] - self.data.values[
            self.data.mask_y_rad_rbv]
        xRad = 2*self.data.values[self.data.mask_x_rad_rbv]
        yRad = 2*self.data.values[self.data.mask_y_rad_rbv]
    ...
    def update_latest_dir(self,widget, value, dummy):
        if self.data.values.get(value) != 'UNKNOWN': # MAGIC_STRING
            s = self.data.values.get(self.data.image_save_dir_root) + '\\' + \
    ...
    def set_widget_dicts(self):
        ...
        self.widget_to_dataname[self.x_val]=  data.x_val
        self.widget_to_dataname[self.x_val_2]=  data.x_val
        self.widget_to_dataname[self.x_mean]= data.x_mean
        self.widget_to_dataname[self.x_sd]= data.x_sd
        self.widget_to_dataname[self.y_val]= data.y_val
        self.widget_to_dataname[self.y_val_2]= data.y_val
        self.widget_to_dataname[self.y_mean]=data.y_mean
        self.widget_to_dataname[self.y_sd]=data.y_sd
        self.widget_to_dataname[self.sx_val]=data.sx_val
        self.widget_to_dataname[self.sx_mean]=data.sx_mean
        self.widget_to_dataname[self.sx_sd]=data.sx_sd
        self.widget_to_dataname[self.sy_val]=data.sy_val
        self.widget_to_dataname[self.sy_mean]=data.sy_mean
        self.widget_to_dataname[self.sy_sd]= data.sy_sd
        self.widget_to_dataname[self.cov_val]= data.cov_val
        self.widget_to_dataname[self.cov_mean]= data.cov_mean
        self.widget_to_dataname[self.cov_sd]= data.cov_sd
        self.widget_to_dataname[self.avg_pix_val]= data.avg_pix_val
        self.widget_to_dataname[self.avg_pix_mean]= data.avg_pix_mean
        self.widget_to_dataname[self.avg_pix_sd]= data.avg_pix_sd
        self.widget_to_dataname[self.imageLayout]= data.image
        self.widget_to_dataname[self.mask_x_read]= data.mask_x_rbv
        self.widget_to_dataname[self.mask_y_read]= data.mask_y_rbv
        self.widget_to_dataname[self.mask_x_rad_read]= data.mask_x_rad_rbv
        self.widget_to_dataname[self.mask_y_rad_read]= data.mask_y_rad_rbv
        self.widget_to_dataname[self.opencloseShut1_pushButton]= data.shutter1_open
        self.widget_to_dataname[self.opencloseShut2_pushButton]= data.shutter2_open
        self.widget_to_dataname[self.acquire_pushButton]= data.is_acquiring
        self.widget_to_dataname[self.analyse_pushButton]= data.is_analysing
        self.widget_to_dataname[self.analyse_pushButton_2]= data.is_analysing
        self.widget_to_dataname[self.collectAndSave_pushButton]= data.is_collecting_or_saving
        self.widget_to_dataname[self.useBackground_pushButton]= data.use_background
        self.widget_to_dataname[self.useNPoint_pushButton]= data.use_npoint
        self.widget_to_dataname[self.minPixValue] = data.min_level_rbv
        self.widget_to_dataname[self.maxPixValue] = data.max_level_rbv
        self.widget_to_dataname[self.step_size_read] = data.ana_step_size
        self.widget_to_dataname[self.H_step_read] = data.H_step_read
        self.widget_to_dataname[self.V_step_read] = data.V_step_read
        self.widget_to_dataname[self.wcm_val] = data.wcm_val
        ...
        # self.widget_to_dataname[self.int_mean] = data.int_mean
        # self.widget_to_dataname[self.int_sd] = data.int_sd
        '''
    ...
    def add_camera_image(self):
        ...
        '''
            ...
            row). However, most image data is stored in row-major order (row, column) and will need 
        ...
        self.vc_image.scale(self.data.values[self.data.x_pix_scale_factor],self.data.values[
         self.data.y_pix_scale_factor])
        self.vc_image.setOpts(axisOrder='row-major')
        self.vc_image.setImage(self.data.values[self.data.image])
        ...
        '''
        border = 50
        self.plot_item.setLimits(xMin=0, xMax=self.data.values[self.data.xpix_full],
                                 yMin=0,
                                 yMax=self.data.values[self.data.ypix_full],
                                 minXRange=10, maxXRange=self.data.values[self.data.xpix_full],
                                 minYRange=10,
                                 maxYRange=self.data.values[self.data.ypix_full])
        self.plot_item.setRange(xRange=[-border, self.data.values[self.data.xpix_full]+border], \
                                yRange=[-border, self.data.values[self.data.ypix_full]+border])
        '''
        ...
        x_range = int(ceil(self.data.values[self.data.xpix_full] * self.data.values[
            self.data.x_pix_to_mm]))
        y_range = int(ceil(self.data.values[self.data.ypix_full] * self.data.values[
            self.data.y_pix_to_mm]))
        x_pm = self.data.values[self.data.x_pix_to_mm]
        y_pm = self.data.values[self.data.y_pix_to_mm]
