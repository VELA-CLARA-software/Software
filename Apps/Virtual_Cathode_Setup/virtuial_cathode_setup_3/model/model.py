...
class model():
    ...
    def update_values(self):
        ...
        # DEBUG for testing we don'tlook at the actual image data for max / min
        ...
        model.data.values[data.min_level_rbv] = amin(model.data.values[data.image])
        model.data.values[data.max_level_rbv] = amax(model.data.values[data.image])
        ...
        # put this is on functionfor
        self.update_shutter_state()


    def get_pixel_to_mm_data(self):
        '''
            These lines get overlayed on the image when widget checked
            they are based on the numpixels , and pixels to mm
            set here
        '''
        # model.data.values[data.y_pix_scale_factor]
        # model.data.values[data.x_pix_scale_factor]
        # model.data.values[data.x_pix_to_mm]
        # model.data.values[data.y_pix_to_mm]
        # model.data.values[data.num_pix_x]
        # model.data.values[data.num_pix_y]
    ...
    def update_shutter_state(self):
        if self.shutter1[0].state == shut.SHUTTER_STATE.OPEN:
            model.data.values[data.shutter1_open] = True
        else:
            model.data.values[data.shutter1_open] = False

        if self.shutter2[0].state == shut.SHUTTER_STATE.OPEN:
            model.data.values[data.shutter2_open] = True
        else:
            model.data.values[data.shutter2_open] = False
