...
class gun_condition(QObject):
	...
	def check_IMG_change_is_small(self, img_signal, rf_amp_signal, rf_pulse_width_signal):
		...
		if delta_IMG > self.pv_monitor_not_alarm_values[ img_signal ]:
			print 'delta_IMG = ', delta_IMG, '  pre_spike_img_mean = ', self.pre_spike_img_mean
			if self.is_not_in_cool_down( img_signal ):
				self.pv_time_of_last_alarm[ img_signal ] =  time.time()
				print 'Vac Spike, Dropping RF pulse width to ' + str(self.latest_values[ rf_pulse_width_signal ] - self.vac_spike_RF_pulse_width_drop) + ' and pausing RF Ramp for ' \
	...
	def change_RF_pulse_width(self, rf_pulse_width_signal, value):
		if value > self.maximum_pulse_width:
			...
			print 'setting nominal pulse width'
		elif value < 1.5:
	...
	def main_loop(self):
		...
		if self.can_increase_rf_pulse_width() and self.latest_values['RF_PULSE_LENGTH'] < self.nominal_pulse_width:
				...
				print 'Increasing RF pulse width to: ', self.latest_values[ 'RF_PULSE_LENGTH' ] + self.vac_spike_RF_pulse_width_increase
				...
				print 'No Events for ' + str( time.time() - self.time_of_last_increase ) + '  RF Amp = ' + str( increase )
...
class gun_condition_window(QMainWindow):
	...
	def __init__(self, parent = None):
		...
		self.centralLayout.addWidget(self.gunParametersWidget)
		...
		self.centralLayout.addWidget(self.sp)
