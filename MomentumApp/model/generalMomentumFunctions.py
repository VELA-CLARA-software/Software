...
class Functions():
	def __init__(self):
		...
		caput('VM-EBT-INJ-MAG-HCOR-02:RIRAN', 0)#for virtual machine
	...
	def stepCurrent(self,magnet,step):
		...
		self.magnets.setSI(magnet,setI,MAG.riTolerance,10)#seet the current and it can wait her untill it reaches to within tolerance
	...
	def fixDispersion(self,quad,screen):
		log.info('Fixed Dispersion with '+quad+' on '+screen)
	def findDispersion(self,dipole,screen):
		print('Determinied Dispersion with '+dipole+' and '+screen)
	def calcMomSpread(self):
		print('Calculated Momentum Spread')
